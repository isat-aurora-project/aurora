...
// auto-generate getters/setters for 2-way binding
...
import { createAuth } from '@/services/auth0.service'
...
const initialState = {
  ...
  user: {
    ...
    app_metadata: {},
    /**
     * Editable User Profile. Contains OIDC-compliant fields.
     *
...
const mutations = {
  ...
  [LOGIN_SUCCESS] (state, { accessToken, idToken, idTokenPayload }) {
    ...
    state.abilities = defineAbilitiesFor(state.roles, idTokenPayload.sub)
...
const actions = {
  ...
  updateUserMeta ({ commit, state }, userMeta) {
    ...
    return auth
      .updateUserMeta(state.accessToken, state.user_id, userMeta)
      .then(
   ...
   * @param {Function} options.commit Function to commit mutations
   * @param {Function} options.state  The auth state of the user
   * @param {Object}   paymentMethod  Details about the payment method
   */
...
const getters = {
  ...
  profile: state => ({ ...state.user.user_metadata }),
